...
def preprocess(inp_args):
    ...
    for data_type in ["tr", "cv", "tt"]:
        if not os.path.exists(os.path.join(inp_args.in_dir, data_type)):
            continue
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser("VCTK2mix data preprocessing")
    parser.add_argument(
        "--in_dir", type=str, default=None, help="Directory path of vctk2mmix including tr, cv and tt"
    )
